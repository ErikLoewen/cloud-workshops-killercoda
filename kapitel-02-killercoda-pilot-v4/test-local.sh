...
pass "Kapitel-1-kompatibler Foreground-Index und Paketvalidierung"

"$root/setup.sh" --sandbox "$sandbox" >/tmp/labforge-v4-setup.out
pass "selbstständiges Foreground-Setup ohne Assets"

expect_success "wiederholtes selbstständiges Setup ohne Doppelinstanz" \
...
if grep -q '^HTTP/1.1 200' <<<"$response" &&
  ...
  pass "HTML/CSS/JavaScript-Web-App unter /architektur"
else
  fail "HTML/CSS/JavaScript-Web-App unvollständig"
fi
...
if grep -q '^192.0.2.10 xebico$' "$PILOT_HOSTS_FILE" &&
   ss -ltnp 2>/dev/null | grep 'xebico-dienst' |
     grep -Eq "0\\.0\\.0\\.0:${runtime_port}|\\*:${runtime_port}"; then
  pass "Reset stellt Hosts-Ausgangsblock und Traffic-fähigen Listener auf $runtime_port her"
else
