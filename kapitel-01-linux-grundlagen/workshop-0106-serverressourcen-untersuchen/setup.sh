...
readonly beschwoerung_start="${install_dir}/start-beschwoerung"
...
process_matches() {
  ...
  local actual_comm actual_exe actual_user
  ...
  actual_user="$(stat -c '%U' "/proc/${pid}")"
  [[ "$actual_user" == "$lab_user" ]]
}
...
cat >"$beschwoerung_start" <<'STARTER'
#!/usr/bin/env bash
set -Eeuo pipefail
...
readonly regulator_bin="/usr/local/lib/leuchtturm/lastregler"
readonly regulator_script="/usr/local/lib/leuchtturm/lastregler.sh"

fail() { printf 'Startfehler: %s\n' "$1" >&2; exit 1; }
...
atomic_write() {
  local target="$1" content="$2" mode="${3:-0640}" tmp
  tmp="$(mktemp "${target}.tmp.XXXXXX")"
  printf '%s\n' "$content" >"$tmp"
  chmod "$mode" "$tmp"
...
[[ -d "$state_dir" && ! -L "$state_dir" ]] || fail "Der Workshopstatus fehlt."
exec 9>"$lock_file"
flock 9
[[ ! -e "$activated" ]] || exit 0

mapfile -t existing < <(matching_pids beschwoerung "$beschwoerung_bin")
[[ ${#existing[@]} -eq 0 ]] || fail "Es läuft bereits eine Workshopstörung."
...
mapfile -t worker_pids < <(matching_pids beschwoerung "$beschwoerung_bin")
for _ in {1..30}; do
  ((${#worker_pids[@]} == 1)) && break
  sleep 0.1
  mapfile -t worker_pids < <(matching_pids beschwoerung "$beschwoerung_bin")
done
[[ ${#worker_pids[@]} -eq 1 ]] || fail "beschwoerung wurde nicht genau einmal gestartet."
worker_pid="${worker_pids[0]}"
[[ "$(<"/proc/${worker_pid}/comm")" == "beschwoerung" ]] ||
  fail "Der Prozessname in /proc ist nicht beschwoerung."
[[ "$(ps -o ni= -p "$worker_pid" | tr -d ' ')" == "15" ]] ||
  fail "beschwoerung läuft nicht mit Nice-Wert 15."

runuser -u "$lab_user" -- /usr/bin/setsid --fork "$regulator_bin" \
  "$regulator_script" "$worker_pid" 9>&- </dev/null >/dev/null 2>&1
...
[[ ${#regulator_pids[@]} -eq 1 ]] || {
  ...
  fail "Der Lastregler wurde nicht genau einmal gestartet."
}
...
nice=15"
atomic_write "$activated" "session_id=${session_id}
result=activated" 0644
flock -u 9
STARTER
chmod 0755 "$beschwoerung_start"
chown root:root "$beschwoerung_start"
