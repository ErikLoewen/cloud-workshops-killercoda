...
readonly asset_dir="/opt/labforge-workshop-05-assets"
...
is_our_unit_file() {
  [[ -f "$1" ]] && grep -qx '# LabForge Workshop 5 isolated service' "$1"
}
...
if [[ -e "$worker_path" ]] && ! is_our_script_file "$worker_path" "LabForge Workshop 5 lab-worker launcher"; then
  fail "$worker_path existiert, gehört aber nicht zu diesem Workshop."
fi

if [[ -e "$wrapper_path" ]] && ! is_our_script_file "$wrapper_path" "LabForge Workshop 5 no-pager wrapper"; then
